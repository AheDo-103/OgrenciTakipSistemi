...
namespace AhdYazilim.OgrenciTakip.UI.Win.Functions
{
    public static class YaziyaCevir
    {
        ...
        private static string YazIle(long value)
        {
            var yazi = "";
            var subStr = "" + value;
            ...
            for (var i = basamak - 1; i >= 0; i--)
            {
                if (i != basamak - 1)
                {
                    var d = Convert.ToInt32(subStr.Substring((basamak - i - 2) * 3 + maxBasamak, 3));
        ...
        public static string YaziIleTutar(this decimal value)
        {
            ...
            return YazIle((long)Math.Floor(value)) + "TL, " + YazIle((long)Math.Floor(kurus * 100)) + "Krş.";
        }

        public static string YaziIleVade(this DateTime date)
        {
            var gunYaziIle = YazIle(date.Day);
            var ayYaziIle = date.ToString("MMMM");
            var yilYaziIle = YazIle(date.Year);
